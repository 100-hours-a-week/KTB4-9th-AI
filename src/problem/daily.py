...
async def generate_one(category: Category, trigger: Trigger) -> Problem | None:
    """
    주어진 카테고리로 데일리 문제 하나를 생성한다. 난이도는 무작위로 고른다.
    ...
    return await run_problem_graph(
        random.choice(DAILY_DIFFICULTIES),
        category,
        trigger=trigger,
        purpose=ProblemPurpose.DAILY,
    )


async def generate_daily_problems(trigger: Trigger) -> list[Problem]:
    """
    ...
    Parameters:
        trigger (Trigger): API 호출이면 ON_DEMAND, 새벽 배치면 BATCH
    ...
    for round_no in range(1, MAX_ROUNDS + 1):
        ...
        results = await asyncio.gather(*(generate_one(c, trigger) for c in picked))
